...
class ProduceThread extends Thread {
	...
	// 构造器参数是生产线程要放入数据的队列
	ProduceThread(List<Object> shareList) {
		...
		while (true) {
			try {
				...
				synchronized (shareList) {
					System.out.println("老板:现在还剩【" + shareList.size()
							+ "】串。需求是[" + ProduceThread.requirement + "]串。");
					...
					System.out.println();
					System.out.println("老板:我这次烤了【" + num + "】串，现在还剩【"
							+ shareList.size() + "】串。需求是["
							+ ProduceThread.requirement + "]串。");
					// 通知消费线程,队列中有对象了
					shareList.notify();
					//
				}
