...
/*
         注意：一个线程被创建后，总是处于一下几种状态之一。Thread类的getState()方法获得当前线程对象的状态。
 1). 新建态： 使用new运算符创建一个线程对象后，该线程仅仅是一个空的对象，系统没有为它分配资源，该线程处于新建态。
 2). 就绪态和运行态：处于新建态的线程启动后，进入就绪态，再有系统调度执行而成为运行态。由于线程调度有系统控制和管理，程序无法控制。
