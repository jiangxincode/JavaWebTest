			...
			if (f.jlc4.isSelected()) {
				...
				if (f.jlc3.isSelected()) {
					...
					if (f.jcx4.getText().equals("")) {
						javax.swing.JOptionPane.showMessageDialog(f.jp3,
								"请输入文件编号");
					} else {
						try {
							...
							for (int count = 0; count < f.vj.size(); count++) {
								...
								if (f.vj.get(count).isSelected()) {
									// 调用时间改名函数
									ReNameByTime(new File(f.vj.get(count)
											.getText()), num);
									num++;
								}
							}
					...
					for (int count = 0; count < f.vj.size(); count++) {
						if (f.vj.get(count).isSelected()) {
							// num算为从零开始
							ReNameByTime(new File(f.vj.get(count).getText()),
									num);
							num++;
						}
					}
							...
							try {
								...
								for (int count = 0; count < f.vj.size(); count++) {
									if (f.vj.get(count).isSelected()) {
										...
										ReName(new File(f.vj.get(count)
												.getText()), filer, "" + num,
												"");
										num++;
									}
						...
						for (int count = 0; count < f.vj.size(); count++) {
							...
							if (f.vj.get(count).isSelected()) {
								...
								if (filer.equals("")) {
									...
									// 调用改名函数
									ReName(new File(f.vj.get(count).getText()),
											filer, "", "");
								}
							}
						}
