 ...
 * @data 2014-6-20
 */
public class Decimal2ChineseCharacter {

	/**
	 * 将人民币数额从数值格式转换到汉字格式。
	 * <p>
	 * 例如人民币数额的数值格式为 123.45，转换成汉字格式之后为壹佰贰拾叁元肆角伍分。
	 * @param value 数值格式的人民币数额
	 * @return
	 */
	public static String convert(String value) {
		return cleanZero(splitNum(roundString(value)));
	}

	/**
	 * 判断用户输入的人民币数额是否合法。
	 * <p>
	 * 用户只能输入大于零的人民币数额，不能输入其它字符
	 * @param inputValue
	...
	public static boolean isValid(String inputValue) {
		...
		try {
			float f = Float.valueOf(inputValue);
			if (f < 0) {
				result = false;
			} else {
				result = true;
	 ...
	 * 把传入的数转换为中文金额大写形式
	 *
	 * @param flag 标志位，1 表示转换整数部分，0 表示转换小数部分
	 * @param s 要转换的字符串
	...
	/**
	 * 把已经转换好的中文金额大写形式加以改进，清理这个字符串里面多余的零，让这个字符串变得更加可观 注：传入的这个数应该是经过 splitNum()
	 * 方法进行处理，这个字符串应该已经是用中文金额大写形式表示的
	 *
	 * @param s 已经转换好的字符串
	 * @return 改进后的字符串
	 */
