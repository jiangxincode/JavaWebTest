...
class CustomerThread extends Thread {
	...
	CustomerThread(List<Object> shareList) {
				...
				synchronized (shareList) {
					if (ProduceThread.requirement < 5) {
						...
						if (num > 3) {
							...
							System.out.println("顾客:老板再来[" + num + "]串。还缺["
									+ ProduceThread.requirement + "]串。现在烤好了【"
									+ shareList.size() + "】串");
						}
					}
					...
					for (int i = 0; i < ProduceThread.requirement && i < cnt; i++) {
						System.out
								.print(" " + ((Kebab) shareList.remove(0)).id);
						x++;
					}
					System.out.println();
					ProduceThread.requirement -= x;
					System.out.println("顾客:我拿走了【" + x + "】串,缺["
							+ ProduceThread.requirement + "]串。现在烤盘里还剩【"
							+ shareList.size() + "】串");
					shareList.notify();
				}
			} catch (Exception ef) {
